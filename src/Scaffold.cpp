...
namespace OptCuts {
Scaffold::Scaffold(OptimizationState& optState)
    : optState(optState)
    , airMesh(optState)
{
}

Scaffold::Scaffold(OptimizationState& optState, const TriMesh& mesh, Eigen::MatrixXd UV_bnds, Eigen::MatrixXi E, const Eigen::VectorXi& p_bnd)
    : optState(optState)
    , airMesh(optState)
{
...
Scaffold& Scaffold::operator=(const Scaffold& scaffold)
{
    airMesh = scaffold.airMesh;
    bnd = scaffold.bnd;
    localVI2Global = scaffold.localVI2Global;
    meshVI2AirMesh = scaffold.meshVI2AirMesh;
    wholeMeshSize = scaffold.wholeMeshSize;
    optState = scaffold.optState;
...
void Scaffold::augmentGradient(Eigen::VectorXd& gradient, const Eigen::VectorXd& gradient_scaf, double w_scaf) const
{
    ...
    if (w_scaf == 0.0) {
        ...
        gradient.bottomRows(gradient.size() - systemSize0) = w_scaf * gradient_scaf.bottomRows(gradient_scaf.size() - bnd.size() * 2);
    }
}
    ...
    double w_scaf) const
{
    ...
    for (int localVI = 0; localVI < localVI2Global.size(); localVI++) {
        ...
        for (int localVJ = 0; localVJ < localVI2Global.size(); localVJ++) {
            ...
            P.block(_2globalVI, _2globalVJ, 2, 2) += w_scaf * P_scaf.block(localVI * 2, localVJ * 2, 2, 2);
        }
    }
}
    ...
    Eigen::MatrixXd& UV, Eigen::MatrixXi& E, Eigen::VectorXi& bnd) const
{
    ...
    for (int vI = 0; vI < loop.size(); vI++) {
        if (loop[vI] == corner[0]) {
            if ((loop[(vI - 1 + loop.size()) % loop.size()] == corner[1]) && (loop[(vI - 2 + loop.size()) % loop.size()] == corner[2])) {
                startI = vI;
                break;
            } else {
