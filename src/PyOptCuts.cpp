    ...
    if (checkConvergence) {
        ...
        if (measure_bound > upperBound) {
            ...
            if ((!checkCand(energyChanges_iSplit)) && (!checkCand(energyChanges_bSplit))) {
                ...
                logFile << "iterativelyUpdated = " << energyParams[0] << ", increased, current eDec = " << eDec_b << ", " << eDec_i << "; id: " << id_pickingBSplit << ", " << id_pickingISplit << std::endl;
            }
        } else {
            bool noOp = true;
...
std::tuple<Eigen::MatrixXd, Eigen::MatrixXd, Eigen::MatrixXi, Eigen::MatrixXi> optimize_mesh(Eigen::MatrixXd& vertices, Eigen::MatrixXd& uvs, Eigen::MatrixXi& indices, Eigen::MatrixXi& uv_indices)
{
    ...
    // Argument 5: methodType
    methodType = OptCuts::MethodType::MT_OPTCUTS;
