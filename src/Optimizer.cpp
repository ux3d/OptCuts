...
Optimizer::Optimizer(OptimizationState& optState, const TriMesh& p_data0,
...
void Optimizer::precompute(void)
{
    result = data0;
    if (scaffolding) {
        scaffold = Scaffold(optState, result, UV_bnds_scaffold, E_scaffold, bnd_scaffold);
...
void Optimizer::updateEnergyData(bool updateEVal, bool updateGradient, bool updateHessian)
{
    ...
    if (updateGradient) {
        ...
        if (gradient.squaredNorm() < targetGRes) {
            std::cout << "||g||^2 = " << gradient.squaredNorm() << " after fracture initiation!" << std::endl;
        }
    }
...
{
    assert(optState.methodType == MT_OPTCUTS);

    topoIter++;

    optState.timer.start(0);
    bool isMerge = false;
...
bool Optimizer::lineSearch(void)
{
    ...
    while (testingE > lastEnergyVal) // ensure energy decrease
    {
        ...
        if (stepSize == 0.0) {
            stopped = true;
            if (!mute) {
                std::cout << "testingE" << globalIterNum << " " << testingE << " > " << lastEnergyVal << std::endl;
            }
            break;
        }
...
{
    double E_se;
    result.computeSeamSparsity(E_se, !optState.fractureMode);
    E_se /= result.virtualRadius;

    if (optState.fractureMode) {
