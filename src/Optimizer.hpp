...
protected: // owned data
    ...
    TriMesh result = TriMesh(optState); // intermediate results of each iteration
    TriMesh data_findExtrema = TriMesh(optState); // intermediate results for deciding the cuts in each topology step
    bool scaffolding; // whether to enable bijectivity parameterization
    double w_scaf;
    Scaffold scaffold = Scaffold(optState); // air meshes to enforce bijectivity
...
public: // constructor and destructor
    Optimizer(OptimizationState& optState, const TriMesh& p_data0, const std::vector<Energy*>& p_energyTerms, const std::vector<double>& p_energyParams,
