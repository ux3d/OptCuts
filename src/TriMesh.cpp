...
TriMesh::TriMesh(const TriMesh& other)
    ...
    , F(other.F)
    , cohE(other.cohE)
    , initSeams(other.initSeams)
    , scaffold(other.scaffold)
    , areaThres_AM(other.areaThres_AM)
    , boundaryEdge(other.boundaryEdge)
    , edgeLen(other.edgeLen)
    , LaplacianMtr(other.LaplacianMtr)
    , triArea(other.triArea)
    , triNormal(other.triNormal)
    , surfaceArea(other.surfaceArea)
    , triAreaSq(other.triAreaSq)
    , e0dote1(other.e0dote1)
    , e0SqLen(other.e0SqLen)
    , e1SqLen(other.e1SqLen)
    , e0SqLen_div_dbAreaSq(other.e0SqLen_div_dbAreaSq)
    , e1SqLen_div_dbAreaSq(other.e1SqLen_div_dbAreaSq)
    , e0dote1_div_dbAreaSq(other.e0dote1_div_dbAreaSq)
    , avgEdgeLen(other.avgEdgeLen)
    , virtualRadius(other.virtualRadius)
    , validSplit(other.validSplit)
    , fixedVert(other.fixedVert)
    , bbox(other.bbox)
    , vertWeight(other.vertWeight)
    , edge2Tri(other.edge2Tri)
    , vNeighbor(other.vNeighbor)
    , cohEIndex(other.cohEIndex)
    , fracTail(other.fracTail)
    , curFracTail(other.curFracTail)
    , curInteriorFracTails(other.curInteriorFracTails)
    , initSeamLen(other.initSeamLen)
    , optState(other.optState)
{
}

TriMesh::TriMesh(OptimizationState& optState)
    : optState(optState)
{
    initSeamLen = 0.0;
}

TriMesh::TriMesh(OptimizationState& optState, const Eigen::MatrixXd& V_mesh, const Eigen::MatrixXi& F_mesh,
...
TriMesh::TriMesh(OptimizationState& optState, Primitive primitive, double size, double spacing, bool separateTri)
    : optState(optState)
{
    switch (primitive) {
    case P_SQUARE: {
    ...
    if (separateTri) {
        *this = TriMesh(optState, V_rest, F, V);
    } else {
        computeFeatures(false, true);
    }
    initSeamLen = 0.0;
...
void TriMesh::computeLaplacianMtr(void)
{
    ...
    for (int k = 0; k < L.outerSize(); ++k) {
        for (Eigen::SparseMatrix<double>::InnerIterator it(L, k); it; ++it) {
            if ((fixedVert.find(static_cast<int>(it.row())) == fixedVert.end()) && (fixedVert.find(static_cast<int>(it.col())) == fixedVert.end())) {
...
{
    optState.timer_step.start(6 + splitInterior);
    ...
    if (!propagate) {
        ...
        if (splitInterior) {
            ...
            optState.inSplitTotalAmt = sortedCandVerts_in.size();
        } else {
            for (int vI = 0; vI < V_rest.rows(); vI++) {
                if (vNeighbor[vI].size() <= 2) {
                    // this vertex is impossible to be splitted further
        ...
        if (!splitInterior) {
            ...
            int bestCandAmt_in = static_cast<int>(std::pow(sortedCandVerts_in.size(), optState.filterExp_in));
            if (bestCandAmt_in < 2) {
                bestCandAmt_in = 2;
            }
...
// #define PROPAGATE_MULTIPLE_TAIL 1
    ...
    if (!splitInterior) {
        if (propagate) {
            ...
            operationType = 0;
            optState.paths_bSplit.resize(0);
            optState.paths_bSplit.resize(bestCandVerts.size());
            optState.newVertPoses_bSplit.resize(bestCandVerts.size());
            optState.energyChanges_bSplit.resize(bestCandVerts.size());
            tbb::parallel_for(0, (int)bestCandVerts.size(), 1, [&](int candI) {
                EwDecs[candI] = computeLocalLDec(bestCandVerts[candI],
                    lambda_t, optState.paths_bSplit[candI],
                    optState.newVertPoses_bSplit[candI],
                    optState.energyChanges_bSplit[candI]);
            });
        }
    } else {
        assert(!propagate);
        ...
        optState.paths_iSplit.resize(0);
        optState.paths_iSplit.resize(bestCandVerts.size());
        optState.newVertPoses_iSplit.resize(bestCandVerts.size());
        optState.energyChanges_iSplit.resize(bestCandVerts.size());
        tbb::parallel_for(0, (int)bestCandVerts.size(), 1, [&](int candI) {
            EwDecs[candI] = computeLocalLDec(bestCandVerts[candI],
                lambda_t, optState.paths_iSplit[candI],
                optState.newVertPoses_iSplit[candI],
                optState.energyChanges_iSplit[candI]);
    ...
    case 0:
        path_max = optState.paths_bSplit[candI_max];
        newVertPos_max = optState.newVertPoses_bSplit[candI_max];
        energyChanges_max = optState.energyChanges_bSplit[candI_max];
        break;

    case 1:
        path_max = optState.paths_iSplit[candI_max];
        newVertPos_max = optState.newVertPoses_iSplit[candI_max];
        energyChanges_max = optState.energyChanges_iSplit[candI_max];
        break;
    ...
    std::pair<double, double>& energyChanges_max)
{
    ...
    optState.timer_step.start(8);

    std::cout << "evaluate edge merge, " << cohE.rows() << " cohesive edge pairs." << std::endl;
    localEwDec_max = -__DBL_MAX__;
    if (!propagate) {
        optState.paths_merge.resize(0);
        optState.newVertPoses_merge.resize(0);
        optState.energyChanges_merge.resize(0);
    }
    for (int cohI = 0; cohI < cohE.rows(); cohI++) {
        int forkVI = 0;
        if (cohE(cohI, 0) == cohE(cohI, 2)) {
        ...
        double localEwDec = computeLocalLDec(0, lambda, path,
            ...
            triangles, mergedPos);
        if (!propagate) {
            optState.paths_merge.emplace_back(path);
            optState.newVertPoses_merge.emplace_back(newVertPos);
            optState.energyChanges_merge.emplace_back(energyChanges);
        }
    ...
    const Eigen::MatrixXd& newVertPos, bool allowCutThrough)
{
    ...
    if (allowCutThrough && (isFromBound || isToBound)) {
        ...
        for (int vI = 0; vI + 1 < path.size(); vI++) {
            ...
            if ((edge2Tri.find(std::pair<int, int>(vInd_s, vInd_e)) == edge2Tri.end()) || (edge2Tri.find(std::pair<int, int>(vInd_e, vInd_s)) == edge2Tri.end())) {
                continue;
            }
            Eigen::MatrixXd newVertPos;
            if (cutThrough) {
...
void TriMesh::computeSeamSparsity(double& sparsity, bool triSoup) const
{
    ...
    for (int cohI = 0; cohI < cohE.rows(); cohI++) {
        if (!boundaryEdge[cohI]) {
            if ((!triSoup) || ((V.row(cohE(cohI, 0)) - V.row(cohE(cohI, 2))).norm() / avgEdgeLen > thres) || ((V.row(cohE(cohI, 1)) - V.row(cohE(cohI, 3))).norm() / avgEdgeLen > thres)) {
...
bool TriMesh::isBoundaryVert(int vI) const
{
    ...
    for (const auto vI_neighbor : vNeighbor[vI]) {
        if ((edge2Tri.find(std::pair<int, int>(vI, vI_neighbor)) == edge2Tri.end()) || (edge2Tri.find(std::pair<int, int>(vI_neighbor, vI)) == edge2Tri.end())) {
            return true;
        }
    }
    ...
    const Eigen::RowVector2d& initMergedPos) const
{
    if (!path_max.empty()) {
        ...
        if (path_max.size() == 3) {
            // zipper merge
            double seDec = (V_rest.row(path_max[0]) - V_rest.row(path_max[1])).norm() / virtualRadius * (vertWeight[path_max[0]] + vertWeight[path_max[1]]) / 2.0;
    ...
    if (isBoundaryVert(vI, *(vNeighbor[vI].begin()), umbrella, boundaryEdge, false)) {
        ...
        for (int startI = 0; startI + 1 < umbrella.size(); startI++) {
            ...
            for (int endI = startI + 1; endI < umbrella.size(); endI++) {
                ...
                const double seInc = ((V_rest.row(path[0]) - V_rest.row(path[1])).norm() * (vertWeight[path[0]] + vertWeight[path[1]]) + (V_rest.row(path[1]) - V_rest.row(path[2])).norm() * (vertWeight[path[1]] + vertWeight[path[2]])) / virtualRadius / 2.0;
    ...
    int maxIter) const
{
    ...
    TriMesh localMesh(optState, localV_rest, localF, localV, Eigen::MatrixXi(), false);
    ...
    Optimizer optimizer(optState, localMesh, energyTerms, energyParams, 0, true, isBijective,
        UV_bnds, E, bnd, true);
    ...
    int maxIter) const
{
    ...
    std::vector<OptCuts::Energy*> energyTerms(1, &SD);
    std::vector<double> energyParams(1, 1.0);
    Optimizer optimizer(optState, localMesh, energyTerms, energyParams, 0, true, !!scaffold,
        UV_bnds, E, bnd, true);
    optimizer.precompute();
    optimizer.setRelGL2Tol(1.0e-6);
