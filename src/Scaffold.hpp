...
public:
    Scaffold(OptimizationState& optState);
    Scaffold(const Scaffold& scaffold);

    Scaffold(OptimizationState& optState, const TriMesh& mesh, Eigen::MatrixXd UV_bnds = Eigen::MatrixXd(),
        Eigen::MatrixXi E = Eigen::MatrixXi(), const Eigen::VectorXi& p_bnd = Eigen::VectorXi());

    Scaffold& operator=(const Scaffold& scaffold);
